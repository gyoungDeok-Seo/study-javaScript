...
getFullName("서", "경덕", function (result) {
  console.log(`${result}님`);
});

// 상품 가격과 총 합을 입력받고 개수를 알아낸 뒤
// 개수가 5개 이하라면 true 아니면 false
function getCount(price, sum, callback) {
  if (callback) {
    return callback(sum / price);
  }
  return sum / price;
}

console.log(getCount(1000, 5000));
// console.log(getCount(1000, 5000, (result) => (result <= 5 ? true : false)));
...
const prin = getPayStatus("키보드", true, (name, message) =>
